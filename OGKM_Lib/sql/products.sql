...
WHERE category <>'surrounding' ORDER BY shelfDate DESC;
SELECT id, name, singer, category, unitPrice, photoUrl, description, shelfDate, discount, stock, Sales FROM products
...
SELECT  id, name, singer, category, unitPrice, photoUrl, description, shelfDate, discount, stock, Sales FROM products  
WHERE name LIKE'' OR singer LIKE '%YOASOBI%' AND category <>'surrounding'  ;

SELECT  id, name, singer, category, unitPrice, photoUrl, description, shelfDate, discount, stock, Sales FROM ogk.products  
WHERE name LIKE'三原色' AND singer LIKE '%YOASOBI%' AND category <>'surrounding'  ;
...
WHERE id AND category <>'surrounding'ORDER BY shelfDate DESC limit 20 ;;

SELECT id, name, singer, category, photoUrl, shelfDate FROM products
WHERE ((id='1' AND singer LIKE '%ADO%') AND  category <>'surrounding') AND id<>'1' Order by shelfDate desc limit 2 ;

SELECT id, name, singer, category, photoUrl, shelfDate FROM products
WHERE  singer LIKE '%ADO%' AND  category <>'surrounding' AND id<>'1' Order by shelfDate desc limit 3 ;
...
WHERE category <>'surrounding' order by Sales Desc limit 10 ;
...
SELECT id, name, singer,SUM(Sales) as singerSales,curdate() - INTERVAL 14 day
 FROM products WHERE category <>'surrounding'GROUP BY singer order by singerSales Desc limit 10;

SELECT distinct id, count(id) as totalp,
FLOOR((count(id)/10)) as pages, 
MOD(count(id),10) as lastpage FROM products

WHERE category <>'surrounding' AND id<13 GROUP BY id  order by id desc limit 10;
