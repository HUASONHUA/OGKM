...
SELECT id, name, singer, category, products.unitPrice, description,
			...
			products.photoUrl,product_surrounding.colorphotourl AS surrounding_photoUrl
			 FROM ogk.products LEFT JOIN ogk.product_surrounding ON products.id=product_surrounding.product_id
			LEFT JOIN ogk.product_surrounding_sizes ON products.id=product_surrounding_sizes.product_id
